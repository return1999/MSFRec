...
sequential_data_list = ['Beauty','Sports_and_Outdoors','Toys_and_Games','Yelp','movielen1m']
session_based_data_list = ['nowplaying','retailrocket','tmall','yoochoose']



#REC 设置随机种子确保实验的可重复性
#REC 模型训练中存在很多随机值，常见的有随机权重、随机数据增强、随机数据读取，这些随机很容易影响网络的训练结果
#REC 我们可以设置随机种子，
def set_seed(seed):
    random.seed(seed)
    ...
    # REC 用于保证CUDA卷积运算的结果确定
    torch.backends.cudnn.deterministic = True
#REC 检查指定的路径（path）是否存在，如果不存在，则创建它，并打印消息通知路径已被创建
def check_path(path):
...
#REC 序列 数据调用函数  返回用户交互序列 用户数目 最大项目ID 和 评估的负样本序列
def get_user_seqs_and_sample(data_file, sample_file):
    lines = open(data_file).readlines()#REC 每一行代表一个用户的交互
    user_seq = []
    item_set = set() #REC set不包含重复项
    for line in lines:
        #REC 数据集中的一行为1 1 2 3 4 5 这行中开头的1表示用户id 12345表示交互的项目ID序列
        #REC 因此要在第一个空格处分割一次得到用户id 和 它交互的项目ID序列
        user, items = line.strip().split(' ', 1)
        #REC 分割所有空格得到项目ID列表
        items = items.split(' ')
        #REC 将项目ID由字符串转换成整数，得到新的列表
        items = [int(item) for item in items]
        user_seq.append(items)
        #REC |是并集的意思 所有不重复的项目ID
        item_set = item_set | set(items)
    max_item = max(item_set)
    num_users = len(lines)
    ...
    #REC 确保user_seq的数量与sample_seq的数量相匹配。
    #REC user_seq 用户历史交互记录，这些数据反映了用户的历史行为和偏好，是构建推荐系统的基础
    #REC sample_seq 用于评估模型性能的负样本数据 99个
...
#REC 模型的评分
#REC pred_list 是一个列表，其中包含了每个样本中真实答案的排名信息
...
def get_seq_dic(args):
    ...
    if args.data_name in sequential_data_list:
        ...
        #REC user_seq 用户交互序列 max_item 最大的项目ID（用于定义模型维度）
        #REC num_users 数据集中的用户数量 sample_seq 用于测试或评估的样本序列
        ...
        # REC 验证集用户交互序列 验证集中的用户数量 用于评估的样本序列
...
#REC 获取模型训练、验证、测试所需要的数据加载器
def get_dataloder(args,seq_dic):

    if args.data_name in sequential_data_list:
        train_dataset = FMLPRecDataset(args, seq_dic['user_seq'], data_type='train')
        #REC RandomSampler有三个参数 data_source（数据集） replacement（是否允许放回抽样，默认为False） num_samples（指定采样的数量，默认为整个数据集的长度）
        #REC 对数据集中的样本进行随机采样，打乱样本顺序
        #REC 每个样本在每个epoch中只会被选中一次
        #REC RandomSampler通常与DataLoader结合使用 DataLoader能够将采样器提供的索引转换成实际的数据样本，并进行批处理
        train_sampler = RandomSampler(train_dataset)

        #REC DataLoader负责在模型训练过程中提供数据
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq'], test_neg_items=seq_dic['sample_seq'], data_type='valid')
        #REC SequentialSampler顺序采样不会打乱数据集中样本的顺序
        #REC 模型的评估阶段，通常不需要打乱数据顺序
